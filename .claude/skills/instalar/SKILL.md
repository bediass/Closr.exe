...
name: instalar
description: >
  Instala o Closr.exe no negócio do usuário. Entrevista sobre empresa, tom de voz,
  ...
  pedir explicitamente "rodar /instalar", "instalar o Closr.exe", "primeiro setup".
---

# /instalar — Instalação inicial do Closr.exe
...
Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `closr.exe`, `Closr.exe`, `Closr.exe-main`, `closr.exe-main` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio, não 'Closr.exe'. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar VS Code, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
