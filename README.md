...
Você acaba de instalar o Closr.exe. Em alguns minutos, sua empresa vai
...
```
Clona o https://github.com/bediass/Closr.exe.git na pasta atual,
...
```
git clone https://github.com/bediass/Closr.exe.git
cd Closr.exe
code .
```
...
Quando o `/instalar` terminar, renomeia a pasta `Closr.exe/` pro nome do teu
negócio (fecha o VS Code, renomeia no Explorer/Finder, abre de novo). A
pasta não fica como "Closr.exe" — ela é o teu negócio agora.
...
closed loop dentro do Closr.exe (decide → executa → captura → realimenta →
ajusta sozinho).
...
[github.com/bediass/Closr.exe](https://github.com/bediass/Closr.exe)
